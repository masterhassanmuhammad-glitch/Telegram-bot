from .helpers import active_menus, send_files_and_recreate_menu
...
def register_user_handlers():
    ...
    def cb_open_folder(call):
        ...
        bot.edit_message_text(
            chat_id=user_id, message_id=call.message.message_id,
            text=display_text, reply_markup=make_sub_menu_markup(btn_id, perms['is_admin'])
        )
        ...
        for f_id, f_type in files:
            try:
                if f_type == 'document': bot.send_document(user_id, f_id)
                elif f_type == 'photo': bot.send_photo(user_id, f_id)
                elif f_type == 'audio': bot.send_audio(user_id, f_id)
                elif f_type == 'video': bot.send_video(user_id, f_id)
                elif f_type == 'voice': bot.send_voice(user_id, f_id)
    ...
    # استقبال الرسالة من الطالب وإرسالها للمشرفين
